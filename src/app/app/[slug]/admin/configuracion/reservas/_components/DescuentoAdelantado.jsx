...
export default function DescuentoAdelantado({ settings, action }) {
  const [state, formAction, pending] = useActionState(action, undefined)

  return (
    <div className="bg-white rounded-xl shadow-sm border border-gray-100 p-6">
      <h2 className="text-base font-semibold text-gray-900 mb-1">Descuento por pago adelantado</h2>
      <p className="text-sm text-gray-500 mb-5">
        Si lo activas, los clientes verán el precio rebajado al reservar online y pagar en el momento.
        ...
        <div className="flex items-end gap-4">
          <div className="w-40">
            ...
            <div className="relative">
              <input
                type="number"
                name="advance_payment_discount"
                defaultValue={settings?.advance_payment_discount ?? ''}
                min={0}
                max={100}
                placeholder="—"
          ...
          {settings?.advance_payment_discount > 0 && (
            <div className="pb-6">
              <span className="inline-flex items-center gap-1.5 bg-green-50 text-green-700 text-xs font-semibold px-3 py-1.5 rounded-full border border-green-200">
                Activo — {settings.advance_payment_discount}% de descuento
              </span>
            </div>
          )}
        </div>
